...
export function VideoCallScreen({ appointmentData, onEndCall }: VideoCallScreenProps) {
  ...
  const toggleMute = () => {
    setIsMuted(!isMuted);
    toast.success(isMuted ? 'MicrÃ³fono activado' : 'MicrÃ³fono desactivado');
  };

  const toggleVideo = () => {
    setIsVideoOff(!isVideoOff);
    toast.success(isVideoOff ? 'CÃ¡mara activada' : 'CÃ¡mara desactivada');
  };
    ...
    <div className="h-screen bg-gray-900 flex flex-col overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-blue-900 via-gray-900 to-purple-900">
          ...
          <div className="absolute top-4 right-4">
            <div className="bg-gray-800/80 backdrop-blur-sm rounded-lg px-3 py-2 flex items-center gap-2">
              ...
              <span className="text-white text-xs">ConexiÃ³n {getConnectionText()}</span>
            </div>
          </div>
        </div>
          ...
          <div className="relative w-full h-full bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center">
            {isVideoOff ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-xs text-gray-500">CÃ¡mara apagada</p>
              </div>
            ) : (
              <div className="flex flex-col items-center">
                ...
                <p className="text-white text-xs mt-2">TÃº</p>
              </div>
            )}
            {isMuted && (
      ...
      <div className="bg-gray-800/95 backdrop-blur-sm px-4 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-center gap-3">
          ...
          <button
            onClick={toggleMute}
            aria-label={isMuted ? 'Activar micrÃ³fono' : 'Silenciar micrÃ³fono'}
          ...
          <button
            onClick={toggleVideo}
            aria-label={isVideoOff ? 'Activar cÃ¡mara' : 'Apagar cÃ¡mara'}
