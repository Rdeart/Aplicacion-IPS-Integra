...
interface HealthMetrics {
  ...
  sleep: number;
  calories: number;
  calorieGoal?: number; // Meta individual de calorÃ­as
}

interface HealthSummaryCardProps {
  metrics: HealthMetrics;
  className?: string;
  onAnalysisClick?: () => void; // Callback para anÃ¡lisis avanzado
}
...
export function HealthSummaryCard({ metrics, className = '', onAnalysisClick }: HealthSummaryCardProps) {
  ...
  // ðŸ§  LÃ³gica de Inteligencia Artificial para analizar mÃ©tricas
  const analyzeHealthMetrics = (data: HealthMetrics): HealthStatus => {
    let outOfRangeCount = 0;
    
    // Rangos de referencia para cada mÃ©trica
    const ranges = {
    ...
    // Determinar estado basado en el anÃ¡lisis de IA
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        ...
        {/* Status - Cambia de color segÃºn el estado */}
        <h3 
        ...
        <p className="text-neutral-500 text-sm">
          AnÃ¡lisis basado en tus mÃ©tricas de salud
        </p>
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center gap-2 text-neutral-500">
            ...
            <span className="text-sm font-medium">MÃ©tricas actualizadas</span>
          </div>
...
// Hook personalizado para obtener anÃ¡lisis detallado (opcional)
export const useHealthAnalysis = (metrics: HealthMetrics) => {
  const getDetailedAnalysis = () => {
    ...
    if (metrics.steps < 8000) {
      issues.push('Actividad fÃ­sica insuficiente');
      recommendations.push('Intenta caminar al menos 8,000 pasos diarios');
    }

    if (metrics.heartRate < 60 || metrics.heartRate > 100) {
      issues.push('Ritmo cardÃ­aco fuera del rango normal');
      recommendations.push('Consulta con tu mÃ©dico sobre tu ritmo cardÃ­aco');
    }

    if (metrics.sleep < 7) {
      issues.push('SueÃ±o insuficiente');
    ...
    if (metrics.calories < calorieGoal * 0.8 || metrics.calories > calorieGoal * 1.2) {
      issues.push('Balance calÃ³rico inadecuado');
      recommendations.push('MantÃ©n un balance calÃ³rico adecuado segÃºn tu meta');
    }

    return {
      issues,
      recommendations,
      overallScore: Math.max(0, 100 - (issues.length * 25)), // PuntuaciÃ³n del 0-100
