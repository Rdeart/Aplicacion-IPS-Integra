...
function AppContent() {
  ...
  const mockUser: UserType = {
    ...
    emergencyContact: {
      name: 'Carlos GarcÃ­a (Esposo)',
      phone: '+57 300 987 6543'
    },
    bloodType: 'O+',
    allergies: ['Penicilina', 'Mariscos'],
    chronicConditions: ['HipertensiÃ³n']
  };
  ...
  const userProfileData = {
    ...
    currentMedications: [
      { name: 'Betahistina 16mg', frequency: 'Tres veces al dÃ­a', time: '08:00, 14:00, 20:00', prescribedBy: 'Dr. Roberto Silva' },
      { name: 'Cetirizina 10mg', frequency: 'Una vez al dÃ­a', time: '20:00', prescribedBy: 'Dr. Carlos Mendoza' }
    ],
    vaccinations: [
      { name: 'COVID-19 (Refuerzo)', date: '2024-09-15', place: 'Centro MÃ©dico Integra' },
      { name: 'Influenza', date: '2024-03-20', place: 'Centro MÃ©dico Integra' },
      { name: 'TÃ©tanos', date: '2023-05-10', place: 'Hospital San JosÃ©' }
    ],
    vitalSigns: {
      weight: '65 kg',
      height: '1.65 m',
  ...
  const devices = [
    { name: 'iPhone 13 Pro', lastActive: 'Ahora', location: 'BogotÃ¡, Colombia', current: true },
    { name: 'iPad Pro', lastActive: 'Hace 2 dÃ­as', location: 'BogotÃ¡, Colombia', current: false },
    { name: 'MacBook Air', lastActive: 'Hace 1 semana', location: 'BogotÃ¡, Colombia', current: false }
  ];
  ...
  const handleLogin = async (email: string, password: string) => {
    ...
    try {
      ...
      setLoginError(msg);
      toast.error('Error de autenticaciÃ³n', { description: msg });
    }
  };
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      setSession(user, accessToken);
      toast.success(`Bienvenid@, ${user.name.split(' ')[0]}!`, {
        description: 'AutenticaciÃ³n biomÃ©trica exitosa',
      });
    } catch (err: any) {
      const msg = err?.message || 'Error en autenticaciÃ³n biomÃ©trica';
      setLoginError(msg);
      toast.error('Error biomÃ©trico', { description: msg });
    }
  };
  ...
  const handleLogout = () => {
    contextLogout();
    toast.info('SesiÃ³n cerrada', { description: 'Has cerrado sesiÃ³n correctamente' });
  };
  ...
  if (isRestoringSession) {
    return <LoadingOverlay message="Restaurando sesiÃ³n..." />;
  }
  ...
  // FunciÃ³n para obtener el tÃ­tulo de cada pantalla
  const getScreenTitle = (screen: string): string => {
    switch (screen) {
      ...
      case 'epidemiological-analysis':
        return 'AnÃ¡lisis EpidemiolÃ³gico';
      case 'blog-detail':
        return 'Blog';
      ...
      case 'profile-policies':
        return 'PolÃ­ticas de Privacidad';
      case 'profile-faq':
  ...
  // FunciÃ³n para determinar si mostrar botÃ³n de retroceso
  ...
  const sidebarNavItems = [
    ...
    { screen: 'profile', icon: User, label: 'ConfiguraciÃ³n', match: ['profile'] },
  ];
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-[#0f1729] to-[#1a2f5a] flex">
        ...
        {/* Sidebar â€” solo desktop, oculta durante videollamada */}
        {activeScreen !== 'video-call' && (
          <aside className="hidden lg:flex flex-col w-64 fixed inset-y-0 left-0 bg-[#171f37] text-white z-40 shadow-2xl">
            ...
            <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto" aria-label="NavegaciÃ³n principal">
        ...
        <div className={`flex-1 flex justify-center items-start ${activeScreen !== 'video-call' ? 'lg:pl-64' : ''}`}>
          <div className="w-full max-w-md lg:max-w-2xl bg-gray-50 relative min-h-screen shadow-2xl overflow-x-hidden">
            ...
            {/* Header â€” oculto durante videollamada */}
            {activeScreen !== 'video-call' && (
              <div className="bg-blue-500 text-white">
                {/* Barra de estado â€” solo mÃ³vil */}
            ...
            {/* NavegaciÃ³n inferior â€” solo mÃ³vil */}
            {activeScreen !== 'video-call' && (
              <div
              ...
              >
                <nav className="flex justify-around items-center" aria-label="NavegaciÃ³n inferior">
                  <button
                  ...
                  <button
                    ...
                    aria-label="ConfiguraciÃ³n" aria-current={activeScreen === 'profile' ? 'page' : undefined}
                  >
                    <User className="h-6 w-6" />
                    <span className="text-xs font-medium mt-1">ConfiguraciÃ³n</span>
                  </button>
                </nav>
              </div>
            )}
